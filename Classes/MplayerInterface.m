...
#else
	myPathToPlayer = @"/usr/local/bin/mplayer";
	[myPathToPlayer retain];
#endif
...
*/	// append additional params
	...
	/* Set a non-obnoxious title */
	[params addObject:@"-title"];
	[params addObject:@"Viewer"];
...
{		
	...
	if (myMplayerTask) {
		...
		case kPaused:
				...
				[self sendCommand:@"osd 0"];
				[self sendCommand:
						[NSString stringWithFormat:@"seek %1.1f %d",seconds, aMode]];
				[self sendCommand:@"osd 1"];
				[self sendCommand:@"pause"];
			break;
		default :
...
- (NSDictionary *) loadInfo
{
	...
	if (myMovieFile)
		[self runMplayerWithParams:[NSArray
				arrayWithObjects:myMovieFile,@"-identify",@"-frames",@"0", nil]];
...
- (void)runMplayerWithParams:(NSArray *)aParams
{
	...
	// set enviroment, actual path
/*	[myMplayerTask setEnvironment:[NSDictionary
			dictionaryWithObject:[myPathToPlayer stringByDeletingLastPathComponent]
			forKey:@"PWD"]];
*/
				...
				if (myUpdateStatistics) {
					...
					case 2:			// only video
						if (sscanf(stringPtr, "V: %f %*d %d%% %d%% %*f%% %d %d %d%%",
								...
								&myPostProcLevel, &myCacheUsage) == 6) {
					...
					case 3:			// only audio
						if (sscanf(stringPtr, "A: %d:%2d:%f %f%% %d%%", &hours, &mins,
								&mySeconds, &audioCPUUsage, &myCacheUsage) == 5) {
		...
		if (strncmp(stringPtr, "ID_", 3) == 0) {
			...
			if (valString != NULL) {
				*valString = '\0';					// replace it with null char
				...
				[myInfo setObject:[NSString stringWithCString:valString] 
						forKey:[NSString stringWithCString:stringPtr]];
			}
			continue; 							// continue on next line	
		}
